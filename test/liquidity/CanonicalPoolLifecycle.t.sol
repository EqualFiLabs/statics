...
contract CanonicalPoolLifecycleTest is CanonicalPoolTestBase {
    ...
    function testCanonicalPoolFeeOverrideIsOwnerControlledAndUsesRegisteredPool() public {
        ...
        emit CanonicalPoolFeeConfigurationSet(
            basketId, assets[0], PoolId.unwrap(pool.poolId), 40, 60, 0, 0, 0, 8_000, 0, 0, 2_000
        );
        basketLiquidity.setCanonicalPoolFeeConfiguration(basketId, assets[0], configuration);
        ...
        assertEq(effective.lockedLiquidityShareBps, 0);
        ...
        assertEq(effective.stonkBrokersShareBps, 0);
        assertEq(effective.indexCreatorShareBps, 0);
        ...
        assertEq(effective.lockedLiquidityShareBps, 1_000);
        assertEq(effective.liquidityProviderShareBps, 2_000);
        assertEq(effective.basketStakerShareBps, 2_000);
        assertEq(effective.staticsStakerShareBps, 1_500);
        assertEq(effective.stonkBrokersShareBps, 1_000);
        assertEq(effective.indexCreatorShareBps, 500);
        assertEq(effective.treasuryShareBps, 2_000);
