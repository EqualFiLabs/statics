...
contract StaticsTestnetFaucetTest is Test {
    ...
    function setUp() public {
        ...
        tokens[1] = new MockERC20("Statics Dollar", "USDstx", 18);
        tokens[2] = new MockERC20("Statics", "STATICS", 18);
        tokens[3] = new MockERC20("Tesla", "TSLA", 18);
        tokens[4] = new MockERC20("Palantir", "PLTR", 18);
        tokens[5] = new MockERC20("AMD", "AMD", 18);
    ...
    function testDeploymentScriptLoadsConfiguredAssetsAndRejectsMissingCode() public {
        ...
        address[6] memory assets = deployment.loadAssets();
        ...
        vm.expectRevert(abi.encodeWithSelector(DeployStaticsTestnetFaucet.InvalidAsset.selector, 5, address(0xBEEF)));
        deployment.loadAssets();

        _configureDeploymentAssets();
        vm.setEnv("STATICS_FAUCET_AMD", vm.toString(address(tokens[4])));
        vm.expectRevert(
            abi.encodeWithSelector(DeployStaticsTestnetFaucet.DuplicateAsset.selector, 5, 4, address(tokens[4]))
        );
        deployment.loadAssets();
    ...
    function testUnderfundedBundleRevertsBeforeAnyTransferOrCooldown() public {
        uint256 available = tokens[5].balanceOf(address(faucet));
        vm.prank(address(faucet));
        tokens[5].transfer(address(1), available);

        vm.expectRevert(
            abi.encodeWithSelector(
                StaticsTestnetFaucet.FaucetUnderfunded.selector, address(tokens[5]), 0, faucet.STOCK_AMOUNT()
            )
        );
        vm.prank(alice);
        faucet.claim();
    ...
    function _configureDeploymentAssets() private {
        ...
        vm.setEnv("STATICS_FAUCET_USDSTX", vm.toString(address(tokens[1])));
        vm.setEnv("STATICS_FAUCET_STATICS", vm.toString(address(tokens[2])));
        vm.setEnv("STATICS_FAUCET_TSLA", vm.toString(address(tokens[3])));
        vm.setEnv("STATICS_FAUCET_PLTR", vm.toString(address(tokens[4])));
        vm.setEnv("STATICS_FAUCET_AMD", vm.toString(address(tokens[5])));
    }

    function _amount(uint256 index) private view returns (uint256) {
        if (index == 0) return faucet.USDG_AMOUNT();
        if (index == 1) return faucet.USDSTX_AMOUNT();
        if (index == 2) return faucet.STATICS_AMOUNT();
