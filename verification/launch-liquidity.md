...
| Exact-input unspecified fees are a fraction of raw pool output; exact-output unspecified fees are grossed up from core input so each configured rate is consistently a fraction of the trader's gross leg | Four Halmos properties covering both directions and exactness modes: symbolic `uint16` amounts when the unspecified delta occupies the low packed half and symbolic `uint8` amounts when it occupies the solver-costlier high packed half; 10,000-case Foundry fuzz over amounts bounded to half of `int128.max` and signed deltas |
