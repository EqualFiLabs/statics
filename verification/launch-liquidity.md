...
| Only PoolManager may enter callbacks; only the owner or an account holding the owner's existing timelock proposer role may register pools; only the owner may change fees or rotate the receiver; ownership cannot be renounced | Halmos and Certora for callback, configuration, and ownership properties; deployment tests for registration authority; fail-on-revert stateful invariant |
