...
| Exact-input unspecified fees are a fraction of raw pool output; exact-output unspecified fees are grossed up from core input so each configured rate is consistently a fraction of the trader's gross leg | A Halmos packed-delta round-trip lemma plus four routing properties isolate the unspecified-leg fee while covering both directions and exactness modes: symbolic `uint16` amounts and fee rates when the unspecified delta occupies the low packed half, plus maximum-amount and maximum-fee branch checks when it occupies the solver-costlier high packed half; 10,000-case Foundry fuzz exercises both configured legs simultaneously across the full allowed fee interval and amounts bounded to half of `int128.max` |
