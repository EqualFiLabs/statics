...
The fresh-deployment launcher installs 21 facets and 190 selectors on
...
installed selector, verify its routed facet and runtime hash, and assert those
fresh-launch totals. Later governed upgrades can change the deployed selector
set; the current deployment manifest records that live release state.
